...
1. Image hosting (free): create a public GitHub repo `fow-card-images`, push `F:/R/fow-card-images`
   to it (`git remote add origin ... && git push -u origin main`), set `GITHUB_USER` in
   `tools/config.py`, then `python tools/build_catalog.py` and `.venv/Scripts/python tools/build_index.py`.
   Images are served by jsDelivr (`https://cdn.jsdelivr.net/gh/<user>/fow-card-images@main/cards/`);
   the extension falls back to the fowsim bucket if a file is missing.
